    ...
    <div className="flex flex-col h-full">
      ...
      {showForm && (
        <div className="px-4 py-3 border-b border-white/[0.06] bg-[#0d1117] space-y-2.5">
          <div className="text-xs font-bold text-blue-400 mb-2">{editId ? "تعديل بيانات العميل" : "إضافة عميل جديد"}</div>
          <div className="grid grid-cols-2 gap-2">
            <input placeholder="الاسم الكامل" value={form.name || ""} onChange={e => setForm({ ...form, name: e.target.value })} className="px-2.5 py-1.5 bg-[#161b22] border border-white/[0.06] rounded-lg text-xs text-white placeholder:text-gray-600 outline-none focus:border-blue-500/30" />
            <input placeholder="رقم الهاتف" value={form.phone || ""} onChange={e => setForm({ ...form, phone: e.target.value })} className="px-2.5 py-1.5 bg-[#161b22] border border-white/[0.06] rounded-lg text-xs text-white placeholder:text-gray-600 outline-none focus:border-blue-500/30" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <input placeholder="البريد الإلكتروني" value={form.email || ""} onChange={e => setForm({ ...form, email: e.target.value })} className="px-2.5 py-1.5 bg-[#161b22] border border-white/[0.06] rounded-lg text-xs text-white placeholder:text-gray-600 outline-none focus:border-blue-500/30" />
            <input placeholder="العنوان" value={form.address || ""} onChange={e => setForm({ ...form, address: e.target.value })} className="px-2.5 py-1.5 bg-[#161b22] border border-white/[0.06] rounded-lg text-xs text-white placeholder:text-gray-600 outline-none focus:border-blue-500/30" />
          </div>
          <textarea placeholder="ملاحظات" value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} className="w-full px-2.5 py-1.5 bg-[#161b22] border border-white/[0.06] rounded-lg text-xs text-white placeholder:text-gray-600 outline-none focus:border-blue-500/30 h-14 resize-none" />
      ...
      <div className="flex-1 overflow-y-auto">
        {filtered.length === 0 ? (
          ...
          <div key={c.id} className="flex items-center gap-3 px-4 py-3 border-b border-white/[0.04] hover:bg-white/[0.02] transition-all">
            <div className="w-9 h-9 rounded-full bg-cyan-900/30 border border-cyan-700/30 flex items-center justify-center text-blue-400 text-sm font-bold shrink-0">
              {c.name.charAt(0)}
            </div>
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => handleEdit(c)} className="p-1 text-gray-600 hover:text-blue-400"><Edit2 size={11} /></button>
