    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-white/[0.05] bg-[#0a1020]">
        ...
        <div className="flex gap-1">
          {(["all", ...Object.keys(CATEGORY_LABELS)] as const).map(c => (
            <button key={c} onClick={() => setFilterCat(c as typeof filterCat)} className={`px-2 py-1 rounded text-[9px] font-semibold transition-all ${filterCat === c ? "bg-cyan-800/40 text-cyan-300 border border-cyan-600/40" : "text-gray-500 hover:text-gray-300"}`}>
      ...
      {showForm && (
        <div className="px-4 py-3 border-b border-white/[0.07] bg-[#0c1428] space-y-2.5">
          <div className="text-xs font-bold text-cyan-400 mb-2">{editId ? "تعديل صنف" : "إضافة صنف جديد"}</div>
          <div className="grid grid-cols-3 gap-2">
            <input placeholder="الاسم (عربي)" value={form.nameAr || ""} onChange={e => setForm({ ...form, nameAr: e.target.value })} className="px-2.5 py-1.5 bg-black/30 border border-white/[0.08] rounded text-xs text-white placeholder:text-gray-600 outline-none focus:border-cyan-700/50" />
            <input placeholder="Name (English)" value={form.name || ""} onChange={e => setForm({ ...form, name: e.target.value })} className="px-2.5 py-1.5 bg-black/30 border border-white/[0.08] rounded text-xs text-white placeholder:text-gray-600 outline-none focus:border-cyan-700/50" />
            <select value={form.category || "other"} onChange={e => setForm({ ...form, category: e.target.value as InventoryItem["category"] })} className="px-2 py-1.5 bg-black/30 border border-white/[0.08] rounded text-xs text-white outline-none">
          ...
          <div className="grid grid-cols-4 gap-2">
            <input placeholder="الماركة" value={form.brand || ""} onChange={e => setForm({ ...form, brand: e.target.value })} className="px-2.5 py-1.5 bg-black/30 border border-white/[0.08] rounded text-xs text-white placeholder:text-gray-600 outline-none focus:border-cyan-700/50" />
            <input placeholder="الموديل" value={form.model || ""} onChange={e => setForm({ ...form, model: e.target.value })} className="px-2.5 py-1.5 bg-black/30 border border-white/[0.08] rounded text-xs text-white placeholder:text-gray-600 outline-none focus:border-cyan-700/50" />
            <input placeholder="الباركود" value={form.barcode || ""} onChange={e => setForm({ ...form, barcode: e.target.value })} className="px-2.5 py-1.5 bg-black/30 border border-white/[0.08] rounded text-xs text-white placeholder:text-gray-600 outline-none focus:border-cyan-700/50" />
            <input placeholder="المورد" value={form.supplier || ""} onChange={e => setForm({ ...form, supplier: e.target.value })} className="px-2.5 py-1.5 bg-black/30 border border-white/[0.08] rounded text-xs text-white placeholder:text-gray-600 outline-none focus:border-cyan-700/50" />
          </div>
          <div className="grid grid-cols-4 gap-2">
            <input type="number" placeholder="الكمية" value={form.quantity ?? ""} onChange={e => setForm({ ...form, quantity: Number(e.target.value) })} className="px-2.5 py-1.5 bg-black/30 border border-white/[0.08] rounded text-xs text-white placeholder:text-gray-600 outline-none focus:border-cyan-700/50" />
            <input type="number" placeholder="سعر الشراء" value={form.costPrice ?? ""} onChange={e => setForm({ ...form, costPrice: Number(e.target.value) })} className="px-2.5 py-1.5 bg-black/30 border border-white/[0.08] rounded text-xs text-white placeholder:text-gray-600 outline-none focus:border-cyan-700/50" />
            <input type="number" placeholder="سعر البيع" value={form.sellPrice ?? ""} onChange={e => setForm({ ...form, sellPrice: Number(e.target.value) })} className="px-2.5 py-1.5 bg-black/30 border border-white/[0.08] rounded text-xs text-white placeholder:text-gray-600 outline-none focus:border-cyan-700/50" />
            <input type="number" placeholder="حد أدنى" value={form.minStock ?? ""} onChange={e => setForm({ ...form, minStock: Number(e.target.value) })} className="px-2.5 py-1.5 bg-black/30 border border-white/[0.08] rounded text-xs text-white placeholder:text-gray-600 outline-none focus:border-cyan-700/50" />
          </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {/* Table header */}
        <div className="grid grid-cols-8 gap-2 px-4 py-2 bg-[#0a1020] border-b border-white/[0.05] text-[9px] text-gray-600 font-bold uppercase tracking-widest sticky top-0">
        ...
        {filtered.length === 0 ? (
          ...
          <div key={item.id} className={`grid grid-cols-8 gap-2 px-4 py-2.5 border-b border-white/[0.03] hover:bg-white/[0.02] transition-all text-[11px] items-center ${item.quantity <= item.minStock ? "bg-red-950/20" : ""}`}>
            ...
            <div className="flex items-center gap-1 justify-end">
              <button onClick={() => handleEdit(item)} className="p-1 text-gray-600 hover:text-cyan-400"><Edit2 size={11} /></button>
